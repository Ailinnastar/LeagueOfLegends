...
## NMAR Analysis
As we mentioned previously that there are some ban columns have missing values, in my opinion this is classified as `not missing at random (NMAR)`.

In professional LOL league, they often do not ban all five of the champions, this sometimes implies that the team have more confidence in winning against the other, or there are no necessity on banning other champions, or even to increases entertainment value for streamers.
...
----
### Check if souls **`'infernals'`**, **`'mountains'`**, **`'clouds'`**, **`'ocean'`**, **`'chemtechs'`**, **`'hextechs'`**  **depends** on **`'patch'`**.

Let `'souls'` be all the souls appears in the data.
...
Therefore, **reject** the null hypothesis, concludes the missingness in `'souls'` **depends** on `'patch'`.
